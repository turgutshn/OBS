...
import com.turggut.sms.iam.domain.RefreshToken;
...
public class AuthService {
    ...
    public AuthResponse login(LoginRequest request, HttpServletRequest httpRequest) {
        ...
        if (!passwordEncoder.matches(request.password(), user.getPasswordHash())) {
            authSecurityService.registerFailedAttempt(user.getId());
    ...
    public AuthResponse refresh(String refreshToken, HttpServletRequest httpRequest) {
        ...
        if (!stored.isActive()) {
            authSecurityService.revokeAllForUser(stored.getUser().getId());
